...
const calculateBedTimes = () => {
    const now = new Date();
    let hours = parseInt(wakeTime.value.hour);
    if (wakeTime.value.ampm === 'AM' && hours === 12) {
        hours = 0;
    } else if (wakeTime.value.ampm === 'PM' && hours !== 12) {
        hours += 12;
    }
    now.setHours(hours);
    now.setMinutes(parseInt(wakeTime.value.minute));

    const bedTimes = Array.from({ length: 4 }, (_, i) => {
        const bedTime = new Date(now);
        const cycles = i + 3;
        bedTime.setMinutes(bedTime.getMinutes() - cycles * 90 - timeToFallAsleep);
        return bedTime;
    });

    results.value = bedTimes;
};
    ...
    <Card class="h-full flex flex-col">
        <CardContent class="flex flex-col h-full p-4">
            <div class="flex items-center gap-2 mb-2">
                ...
                <h3 class="text-sm font-semibold">When should I go to bed?</h3>
            </div>

            <div v-if="!results.length" class="flex flex-col flex-1">
                <p class="text-xs text-muted-foreground mb-3">
                    Enter your wake-up time to see the best times to fall asleep.
                </p>
                ...
                <div class="mt-auto hidden 2xl:flex items-center gap-2 py-2 px-3 bg-muted/50 rounded-lg">
                    ...
                    <div class="text-xs">
                        <p class="font-medium">Why does timing matter?</p>
                        <p class="text-muted-foreground">Waking between sleep cycles helps you feel more refreshed</p>
                    </div>
                </div>
            </div>

            <div v-else class="flex-1 flex flex-col">
                <p class="text-xs text-muted-foreground mb-2">To wake at {{ wakeTime.hour }}:{{ wakeTime.minute }} {{
                    wakeTime.ampm }}, go to sleep at:</p>
